...
module.exports.within = (selector) => {
    ...
    return new RelativeSearchElement(async (e, v) => {
        const rect = await domHandler.getBoundingClientRect(e);
        return (rect.left >= v.left && rect.left <= v.right) &&
        (rect.right <= v.right && rect.right >= v.left) &&
        (rect.top <= v.bottom  && rect.top >= v.top) &&
        (rect.bottom >=  v.top && rect.bottom <= v.bottom);
    }, value, desc);
};
